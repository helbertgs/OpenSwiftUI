...
public struct DismissBehavior : Sendable {
    ...
    /// The internal role of the dismiss behavior.
    ...
    public static let interactive: DismissBehavior = .init(role: .interactive)
    ...
    public static let destructive: DismissBehavior = .init(role: .destructive)

    // MARK: - Creating a dismiss behavior

    /// Creates a dismiss behavior with the given role.
    ///
    /// - Parameter role: The role of the dismiss behavior.
    package init(role: Role) {
        self.role = role
    }
}

extension DismissBehavior {

    /// The role of the dismiss behavior.
